#!/usr/bin/env python3
"""
Example script demonstrating vLLM backend usage.

This script shows how to use the modular backend system with vLLM
...
def tokenize_prompts(prompts, tokenizer_name=TOKENIZATION_CONFIG["tokenizer_name"]):
    ...
    for prompt in prompts:
        ...
        if hasattr(tokenizer, 'apply_chat_template'):
            ...
            # Decode to get the processed string
        ...
        prompt_strings.append(processed_string)
    
    return tokenized, prompt_strings, tokenizer


def example_sync_inference():
    """Example of synchronous inference with vLLM backend."""
    print("=" * 60)
    print("vLLM Synchronous Inference Example")
    print("=" * 60)
    ...
    tokenized_prompts, prompt_strings, tokenizer = tokenize_prompts(prompts)
    
    # Initialize and use backend
    with VLLMBackend() as backend:
...
async def example_async_inference():
    """Example of asynchronous inference with vLLM backend."""
    print("\n" + "=" * 60)
    print("vLLM Asynchronous Inference Example")
    print("=" * 60)
