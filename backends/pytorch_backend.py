...
# Add ref_dsinfer to path
ref_dsinfer_path = os.environ.get('REF_DSINFER_PATH', '/opt/ref_dsinfer/inference')
...
# Hardcoded PyTorch backend configuration
PYTORCH_CONFIG = {
    "model_name": "deepseek-ai/DeepSeek-R1",
    "batch_size": 16,
    "max_seq_len": 3136 + 32 * 1024 + 4, # for chat template
...
class PyTorchBackend(BaseBackend):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        Args:
            config: Optional configuration dict (ignored, uses hardcoded config)
        """
        # Use only hardcoded config
        self.config = PYTORCH_CONFIG.copy()
        ...
        self.config_path = Path('/opt/ref_dsinfer/inference/configs/config_671B.json')
        ...
        if not self.model_path.exists():
            raise RuntimeError(
                ...
                f"Please ensure the model has been downloaded and converted."
            )

        # Check if checkpoint exists for this rank
        checkpoint_file = self.model_path / f"model{self.rank}-mp{self.world_size}.safetensors"
        if not checkpoint_file.exists():
            raise RuntimeError(
                f"Checkpoint not found: {checkpoint_file}. "
                f"Please ensure the model was converted with --model-parallel {self.world_size}"
            )

    def initialize(self) -> None:
        """Initialize the model and tokenizer."""
        if self.rank == 0:
            logger.info(f"Initializing PyTorch backend with {self.world_size} ranks")
        ...
        if not self.config_path.exists():
            raise RuntimeError(f"Model configuration not found at {self.config_path}")
    ...
    def _generate_batch(
        ...
        total_len = min(self.model.max_seq_len, self.config['max_new_tokens'] + max_prompt_len)
        ...
        for i, t in enumerate(prompt_tokens):
            tokens[i, :len(t)] = torch.tensor(t, dtype=torch.long, device=self.config['device'])

        # Generation loop
        prev_pos = 0
        finished = torch.tensor([False] * len(prompt_tokens), device=self.config['device'])
        ...
        for i, toks in enumerate(tokens.tolist()):
            toks = toks[prompt_lens[i]:prompt_lens[i] + self.config['max_new_tokens']]
    ...
    def generate(self, tokenized_prompts: List[List[int]], prompt_strings: Optional[List[str]] = None, **kwargs) -> List[Dict[str, Any]]:
        ...
        Args:
            ...
            prompt_strings: Optional list of prompt strings (not used)
        ...
        if self.world_size > 1 and self.rank != 0:
            raise RuntimeError("generate() should only be called by rank 0 in distributed mode")
