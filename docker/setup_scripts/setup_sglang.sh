...
# Verify SGLang is available (should be from base image)
if python3 -c "import sglang" 2>/dev/null; then
    SGLANG_VERSION=$(python3 -c "import sglang; print(sglang.__version__)" 2>/dev/null || echo "version info not available")
    echo "SGLang is available: $SGLANG_VERSION"
else
    echo "Warning: SGLang not found in the environment"
fi
...
# Verify FlashInfer is available (SGLang dependency)
if python3 -c "import flashinfer" 2>/dev/null; then
    echo "FlashInfer is available (SGLang dependency)"
else
    echo "Warning: FlashInfer not found - may affect SGLang performance"
fi
